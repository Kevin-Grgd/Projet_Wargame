...
public class Carte extends JPanel implements IConfig, ICarte{
    ...
    */
    ...
    public void Sauvegarde(int i){
        //Save
        int numSave = i;

        SaveFile vFile = new SaveFile(numSave);
        ...
        try {
           ...
           out.close();
           fileOut.close();
           System.out.printf("Serialized data is saved in "+vFile.getFileName());
        } catch (IOException e) {
           e.printStackTrace();
        }
    }

    public void Recharger(int i){
        //Load
        int numLoad = i;

        SaveFile vFile = new SaveFile(numLoad);
        try {
            FileInputStream fileIn = new FileInputStream(vFile.getFileName());
            ObjectInputStream in = new ObjectInputStream(fileIn);
            vFile = (SaveFile)in.readObject();
            System.out.printf("Serialized data is load from "+vFile.getFileName());
